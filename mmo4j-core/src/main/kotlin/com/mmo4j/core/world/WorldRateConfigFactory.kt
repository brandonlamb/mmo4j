...
class WorldRateConfigFactory {
  ...
  fun createWorldRate(
    ...
    rest: RestConfig,
    damage: DamageConfig,
    auction: RateConfig.AuctionConfig,
    mining: MiningConfig
    ...
    RateConfig(
      ...
      honor = getDouble("honor"),
      instanceResetTime = getDouble("instance-reset-time"),
      talent = getDouble("talent"),
      rage = rage,
  ...
  @Bean
  @Singleton
  fun createRateRage(config: Config) =
  ...
  @Bean
  @Singleton
  fun createRateRunicPower(config: Config) =
  ...
  @Bean
  @Singleton
  fun createRateSkill(config: Config) =
  ...
  fun createWorldRateXp(config: Config) =
    with(config.getConfig("mmo4j.world.rate.experience")) {
  ...
  @Bean
  @Singleton
  fun createWorldRateDamage(config: Config) =
    with(config.getConfig("mmo4j.world.rate.damage")) {
      DamageConfig(fall = getDouble("fall"))
    }

  @Bean
  @Singleton
  fun createWorldRateAuction(config: Config) =
    with(config.getConfig("mmo4j.world.rate.auction")) {
      RateConfig.AuctionConfig(time = getDouble("time"), deposit = getDouble("deposit"), cut = getDouble("cut"))
    }
}
